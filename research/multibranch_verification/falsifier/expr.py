"""Exact residual / limit helpers for toy polynomial families.

ZERO only on algebraic identity. Infinity, directional disagreement, and
nonzero polynomials are NONZERO. Timeout / failure is UNKNOWN, never ZERO.
"""
...
def _is_exact_nonzero_number(expr: Any) -> bool:
    if expr is None or is_infinity(expr):
...
def substitute_or_limit(expr: Any, var: Any, point: Any) -> tuple[Any, str]:
    """Prefer cancelled substitution; fall back to two-sided limit."""
    if expr is None or var is None or point is None:
        return None, "missing"
    try:
    ...
    try:
        ...
        sub = None
    if sub is not None and not is_undefined(sub) and not is_infinity(sub):
        try:
