...
5. **Certification.** The model writes a proposal, never machine Exact.
   `scripts/certify.py` compiles residuals and writes receipts.
   `EXACT` only for compiled `A − B = 0`. Cited rules stay `CITED_RULE`.
   Remainders stay `ASYMPTOTIC_UNCERTIFIED`. Numerics stay
   `NUMERICAL_SUPPORT`. Without the engine, algebra stays `GAP`.
