...
The product has two workflows, forward derivation and paper audit, over
one deterministic kernel. Engine version `0.3.0` is unchanged from the
0.2.1 preview: `ZERO` remains exact engine `ZERO`.
