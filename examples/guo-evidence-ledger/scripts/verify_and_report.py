...
def load_inventory_numbers() -> list[str]:
    text = (ROOT / "input" / "EQUATION_INVENTORY.yaml").read_text()
...
def main() -> None:
    ...
    rec_dir = ROOT / "expected" / "machine_records"
...
def write_coverage(rows, inventoried, uncovered, n_false, n_ctrl) -> None:
    ...
    (ROOT / "expected" / "COVERAGE.json").write_text(json.dumps(cov, indent=2) + "\n")
...
def write_results(rows, inventoried, uncovered, n_false, n_ctrl) -> None:
    ...
    # Reviewer-facing Markdown is the same frozen table.
