# Reviewer report — Guo et al., PRL 136, 206303

This Markdown report is the same frozen derivation audit as
[`output/index.html`](index.html). Open the HTML for the coloured
appendix map, Sign queue, and residual cards.

**Presentation is not a certificate.** Statuses are copied from
`evidence/RESULTS.md`. JavaScript does not infer ZERO.

| Layer | Role |
|---|---|
| `input/` | Paper source and numbered-equation inventory |
| `evidence/` | Frozen relations, residuals, RESULTS table |
| `output/index.html` | Reviewer-facing HTML |
| `output/REPORT.md` | This file — same scientific table |

Machine-certified means a compiled residual was exact `ZERO` or
`ZERO_UNDER_SUBSTITUTION`. Remainders, special functions, claimed
cancels, and uncompiled algebra stay uncertified. A Sign button is
local UI, not a RESULTS rewrite.

Invalid historical `0*` overlay is archived under
`docs/history/invalid-0star-lineage/` and is not part of this report.
