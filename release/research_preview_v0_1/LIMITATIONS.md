...
Engineering decision for this bounded Mode A preview: `RESEARCH_PREVIEW_ALPHA`
(tag intent `research-preview-v0.1.0-alpha`). That name certifies the
installable researcher-workspace workflow, not representation invention or
autonomous physics discovery.

The historical full pytest suite remains disclosed as
`2049 passed, 24 failed`; those failures are frozen-research-only and are
not repaired in this program. The tested platform is CPython 3.12 on
macOS arm64. The legacy file-oriented `symbols.json` / session CLI is a
compatibility surface and is not the v0.1 workspace contract.
